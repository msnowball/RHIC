...
bool PHPy8ParticleTrigger::Apply(Pythia8::Pythia *pythia) 
{
  ...
  for (int i = 0; i < pythia->event.size(); ++i)
      ...
      for (int j = 0; j < int(_theParticles.size()); j++)
	{
	  ...
	  if(pythia->event[i].id() == _theParticles[j] && pythia->event[i].status() > 0) //only stable particles
	    {
	      if(_doBothEtaCut && (pythia->event[i].eta() < _theEtaLow || pythia->event[i].eta() > _theEtaHigh)) continue;
	      if(_doEtaLowCut && pythia->event[i].eta() < _theEtaLow) continue;
	      if(_doEtaHighCut && pythia->event[i].eta() > _theEtaHigh) continue;

	      if(_doBothAbsEtaCut && (abs(pythia->event[i].eta()) < _theEtaLow || abs(pythia->event[i].eta()) > _theEtaHigh)) continue;
	      ...
	      if(_doBothPtCut && (pythia->event[i].pT() < _thePtLow || pythia->event[i].pT() > _thePtHigh)) continue;
	      if(_doPtHighCut && pythia->event[i].pT() > _thePtHigh ) continue;
	      if(_doPtLowCut && pythia->event[i].pT() < _thePtLow) continue;

	      if(_doBothPCut && (pythia->event[i].pAbs() < _thePLow || pythia->event[i].pAbs() > _thePHigh)) continue;
	      if(_doPHighCut && pythia->event[i].pAbs() > _thePHigh ) continue;
	      if(_doPLowCut && pythia->event[i].pAbs() < _thePLow) continue;

	      if(_doBothPzCut && (pythia->event[i].pz() < _thePzLow || pythia->event[i].pz() > _thePzHigh)) continue;
	      if(_doPzHighCut && pythia->event[i].pz() > _thePzHigh ) continue;
	      if(_doPzLowCut && pythia->event[i].pz() < _thePzLow) continue;

	      if(_verbosity > 5) cout << "stable " << pythia->event[i].id() << "  pt: " << pythia->event[i].pT() << " pz: " << pythia->event[i].pz() << " p: " << pythia->event[i].pAbs() << " eta: " << pythia->event[i].eta() << endl; 
...
void PHPy8ParticleTrigger::AddParents(std::string parents)
{
  ...
  cout << _theParents.size() << "   " << addedParents.size() << endl;
}
