...
#include "mSingleMuRM.h"


using namespace std;

mSingleMuAna::mSingleMuAna(const TString fileName):
//mSingleMuAna::mSingleMuAna(const char* output):
  SubsysReco("mSingleMuAna")
  //  OutputFileName(output)
{
  outFileName = fileName;
  fileCreateMode = "RECREATE";
  fileCompressMode = 100;
  ...
  InitTree(tree);
  //HistoManager->registerHisto(tree);

  InitVariables();

  hists1D_["nEvents"] = new TH1F("nEvents","nEvents",2,0,2);
  //HistoManager->registerHisto(hists1D_["nEvents"]);
  iEvent = 0;
...
int mSingleMuAna::End(PHCompositeNode *top_node)
{
  ...
  TFile *outFile = new TFile(outFileName,fileCreateMode,outFileName,fileCompressMode);
  if(outFile->IsZombie())
    {
