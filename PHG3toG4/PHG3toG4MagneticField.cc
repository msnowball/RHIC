...
PHG3toG4MagneticField::PHG3toG4MagneticField( const string filename)
{    
  _verbosity = 0;

  G4cout << "\n=========== PHG3toG4MagneticField - Begin Construct Mag Field ============" << endl;

  // open file
  TFile *rootinput = new TFile(filename.c_str(),"READ");
  if (!rootinput)
    {
  ...
  for( ; iter!=sorted_map.end(); ++iter ) {
    ...
    if( fabs(z)<10 && ir<10 /*&& iphi==2*/ && _verbosity>3){
...
void PHG3toG4MagneticField::GetFieldValue(const double point[4], double *Bfield ) const
{
  if(_verbosity>2)
    G4cout << "\nPHG3toG4MagneticField::GetFieldValue" << endl;
  double x = point[0];
  double y = point[1];
  double z = point[2];
  ...
  if ( (z>minz_) && (z<maxz_) ) {
    ...
    //G4cout << "mz: " << minz_ << "  " << maxz_ << "  " << z << G4endl;
  ...
  /*
  if(_verbosity>2){ 
    G4cout << "END PHG3toG4MagneticField::GetFieldValue\n" 
           << "  --->  {Bx, By, Bz} : "
...
void PHG3toG4MagneticField::GetFieldCyl( const double CylPoint[4], double *BfieldCyl ) const
{

  //cacheing
  static unsigned int r_index0_cache = 0;
  static unsigned int r_index1_cache = 0;
  static unsigned int z_index0_cache = 0;
  static unsigned int z_index1_cache = 0;
  static unsigned int phi_index0_cache = 0;
  static unsigned int phi_index1_cache = 0;
  ...
  //if( _verbosity>2 ) G4cout << "GetFieldCyl@ <z,r,phi>: {" << z << "," << r << "," << phi << "}" << endl;
  ...
  if (!((z > z_map_[z_index0])&&(z < z_map_[z_index1]))) 
    {
      ...
      if(z_index0 == z_map_.size()-1) z_index0 -= 1;
      z_index1 = z_index0 + 1;
  ...
  unsigned int phi_index0 = phi_index0_cache;
  unsigned int phi_index1 = phi_index1_cache;
  if (!((phi > phi_map_[phi_index0])&&(phi < phi_map_[phi_index1])))
    {
      ...
      if(phi_index1 >= phi_map_.size())	phi_index1 = 0;
      
      phi_index0_cache = phi_index0;
      phi_index1_cache = phi_index1;
    }

  //G4cout << z_index1 << ":"<< r_index0<<":" << phi_index0<< "     (" <<r<<","<<z<<","<<phi<<")" << G4endl; 
...
bool PHG3toG4MagneticField::bin_search( const vector<float>& vec, unsigned start, unsigned end, const float& key, unsigned& index ) const { 
...
void PHG3toG4MagneticField::print_map( map<trio,trio>::iterator& it ) const {
...
#ifndef  __CINT__
PHG3toG4MagneticFieldWrapper::PHG3toG4MagneticFieldWrapper(const string &filename) :
  field(filename)
{}

double PHG3toG4MagneticFieldWrapper::GetBx(double x, double y, double z)
{
...
double PHG3toG4MagneticFieldWrapper::GetBy(double x, double y, double z)
{
  double pos[4] = {x*cm, y*cm, z*cm, 0};
  double Bfield[6] = {0};
  field.GetFieldValue(pos,Bfield);
...
double PHG3toG4MagneticFieldWrapper::GetBz(double x, double y, double z)
{
  double pos[4] = {x*cm, y*cm, z*cm, 0};
  double Bfield[6] = {0};
  field.GetFieldValue(pos,Bfield);
...
double PHG3toG4MagneticFieldWrapper::GetBCylZ(double z, double r, double phi) 
{
...
double PHG3toG4MagneticFieldWrapper::GetBCylR(double z, double r, double phi) 
{
  double pos[4] = {z*cm, r*cm, phi*rad, 0};
  double Bfield[6] = {0};
  field.GetFieldCyl(pos,Bfield);
...
double PHG3toG4MagneticFieldWrapper::GetBCylPHI(double z, double r, double phi) 
{
  double pos[4] = {z*cm, r*cm, phi*rad, 0};
  double Bfield[6] = {0};
  field.GetFieldCyl(pos,Bfield);
